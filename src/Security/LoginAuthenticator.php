...
class LoginAuthenticator extends AbstractLoginFormAuthenticator
{
    ...
    public function authenticate(Request $request): Passport
    {
        ...
        if (!$user) {
            // Si l'utilisateur n'est pas un administrateur, on utilise le badge UserBadge
            return new Passport(
                new UserBadge($email),
                new PasswordCredentials($request->request->get('password', '')),
                [
        ...
        // Si l'utilisateur est un administrateur, on utilise un badge personnalisé AdminBadge
        return new Passport(
            new AdminBadge($email),
            new PasswordCredentials($request->request->get('password', '')),
            [
                new CsrfTokenBadge('authenticate', $request->request->get('_csrf_token')),
                new RememberMeBadge(),
    ...
    public function onAuthenticationSuccess(Request $request, TokenInterface $token, string $firewallName): ?Response
    {
        ...
        if ($token->getUser() instanceof AdminBadge) {
            return new RedirectResponse($this->urlGenerator->generate('admin_dashboard'));
        }

        // Pour les utilisateurs normaux, redirigez-les vers la page des blogs
